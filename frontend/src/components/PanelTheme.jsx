...
const PanelTheme = ({ setIsPanelVisible }) => {
  return (
    <OptionsContainer
      customStyle="absolute right-0 top-[45px] flex w-full flex-col p-5  md:right-2 md:w-[440px]"
      setIsPanelVisible={setIsPanelVisible}
    >
      <button
        className="absolute right-4 rounded-md p-1 text-xl text-text-color-light hover:bg-gray-200 dark:text-text-color-dark dark:hover:bg-hover-color-dark-1"
        onClick={() => setIsPanelVisible(false)}
      >
        <IoMdClose />
      </button>

      <p className="text-start text-[18px] font-bold text-text-color-light dark:text-text-color-dark">
        Themes
      </p>
      ...
      <div className="mt-6 flex flex-col gap-2">
        <p className="text-start text-[13px] text-text-color-light-lite dark:text-text-color-dark-lite">
          My colors
        </p>
